...
void ILI9341_DrawImage(const uint8_t* image, uint8_t orientation)
{
    ...
    {
        width = ILI9341_SCREEN_WIDTH;  // 240
        height = ILI9341_SCREEN_HEIGHT; // 320
    }
    else if (orientation == SCREEN_VERTICAL_1 || orientation == SCREEN_VERTICAL_2)
    {
        width = ILI9341_SCREEN_HEIGHT; // 320
        height = ILI9341_SCREEN_WIDTH;  // 240
    }
    else
    {
        // Invalid orientation
    ...
    ILI9341_SetAddress(0, 0, width - 1, height - 1);

    uint32_t image_size = width * height * 2; // RGB565, 2 bytes per pixel
    uint32_t counter = 0;
    ...
    while (counter + BURST_MAX_SIZE <= image_size)
    {
        ...
        DelayUs(1); // Adjust delay if necessary
    }
...
void ILI9341_DrawMonoImageAt(const unsigned char* image, uint16_t start_x, uint16_t start_y, uint16_t image_width, uint16_t image_height, uint16_t color_fg, uint16_t color_bg)
{
    uint16_t end_x = start_x + image_width - 1;
    uint16_t end_y = start_y + image_height - 1;
    ...
    // Each byte in image contains 8 pixels, MSB first
    uint32_t byte_index = 0;
    for (uint16_t y = 0; y < image_height; y++)
    {
        for (uint16_t x_byte = 0; x_byte < (image_width + 7) / 8; x_byte++)
        {
            uint8_t byte = image[byte_index++];
            for (uint8_t bit = 0; bit < 8; bit++)
            {
                if ((x_byte * 8 + bit) >= image_width) break; // avoid overflow

                if (byte & (0x80 >> bit))
                {
                    // Bit = 1 → Foreground
                    uint8_t data[2] = { color_fg >> 8, color_fg & 0xFF };
                    ILI9341_WriteBuffer(data, 2);
                }
                else
                {
                    // Bit = 0 → Background
                    uint8_t data[2] = { color_bg >> 8, color_bg & 0xFF };
                    ILI9341_WriteBuffer(data, 2);
                }
            }
        }
