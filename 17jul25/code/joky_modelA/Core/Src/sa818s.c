...
static HAL_StatusTypeDef SA818S_SendCommand(const char *cmd, char *response, uint32_t timeout) {
    ...
    // Send command
    if (HAL_UART_Transmit(&SA818_UART_HANDLE, (uint8_t *)cmd, strlen(cmd), 100) != HAL_OK) {
        return HAL_ERROR;
    }

    // Start receiving response
    HAL_UART_Receive_IT(&SA818_UART_HANDLE, rx_buffer, RX_BUFFER_SIZE);
